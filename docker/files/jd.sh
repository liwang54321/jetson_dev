...
function __build_rootfs_impl() {
    ...
    if [ ${is_custom} = "yes" ]; then
        ...
        [ ! -e ${custom_rootfs} ] && __gen_basic_rootfs || exit -1
...
function build_rootfs() {
    ...
    if [ ${is_custom} == "yes" ]; then
        ...
        [ -d ${JETSON_ROOTFS_DEV} ] && sudo rm ${JETSON_ROOTFS_DEV} && sudo mv ${JETSON_ROOTFS} ${JETSON_ROOTFS_DEV}
    fi
...
function parse_args() {
    while [ -n "${1}" ]; do
        ...
        -h | --help)
            ...
            ;;
        -i | --install_sdk)
            install_sdk ${2} && exit 0
            ;;
        --build_kernel)
            build_kernel && exit 0
            ;;
        --build_rootfs)
            build_rootfs ${2} ${3} && exit 0
            ;;
        --install_kernel)
            install_kernel && exit 0
            ;;
        --build_image)
            build_image ${2} && exit 0
            ;;
        -f | --flash)
            flash ${2} && exit 0
            ;;
